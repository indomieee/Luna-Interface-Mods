...
local Luna = { Folder = "SpyXHub", Options = {}, ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(117, 164, 206)), ColorSequenceKeypoint.new(0.50, Color3.fromRGB(123, 201, 201)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(224, 138, 175))} }
...
local configFolder 		= Luna.Folder .. "/configs/" .. gameId
local autoloadPath 		= Luna.Folder .. "/settings/" .. gameId .. "/autoload.txt"
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateButton(ButtonSettings)
				...
				Button.Visible = true
				Button.Parent = ButtonSettings.Parent or Section.Container or TabPage
						...
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									...
									print("Error : "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				Toggle.Visible = true
				Toggle.Parent = ToggleSettings.Parent or Section.Container or TabPage
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Error : "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
					...
					if CheckingForKey then
						if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
							...
							if not Success then
								...
								print("Error : "..BindSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Bind.Title.Text = BindSettings.Name
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								...
								print("Error : "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("Error : "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
				...
				Dropdown.Interact.MouseButton1Click:Connect(function()
					...
					if opened then
						-- OPTIONAL: update dynamic sources first
						if DropdownSettings.SpecialType == "Player" then
							PlayerTableRefresh()
						end
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("Error : "..ColorPickerSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						ColorPicker.Title.Text = ColorPickerSettings.Name
		...
		function Tab:CreateDropdownSection(data)
			...
			function Section:AddToggle(args, flag)
				args.Parent = Content
				return self:CreateToggle(args, flag)
			end

			function Section:AddButton(args)
				args.Parent = Content
				return self:CreateButton(args)
			end

			function Section:AddDropdown(args, flag)
				args.Parent = Content
				return self:CreateDropdown(args, flag)
			end
		...
		function Tab:BuildConfigSection()
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					local configPath = Luna.Folder .. "/settings/" .. gameId .. "/" .. selectedConfig .. ".luna"
		...
		local function BuildFolderTree()
			...
			local paths = {
				...
				Luna.Folder .. "/settings/" .. gameId -- ✅ GAME FOLDER
			}
		...
		function Luna:SaveConfig(Path)
			...
			local fullPath = Luna.Folder .. "/settings/" .. gameId .. "/" .. Path .. ".luna" -- ✅ UPDATED
		...
		function Luna:LoadConfig(Path)
			...
			local file = Luna.Folder .. "/settings/" .. gameId .. "/" .. Path .. ".luna" -- ✅ UPDATED

			if not isfile(file) then return false, "Invalid file (not for this game)" end
		...
		-- AUTOLOAD (GAME-SPECIFIC)
		function Luna:LoadAutoloadConfig()
			if isStudio then return "Config system unavailable." end

			local path = Luna.Folder .. "/settings/" .. gameId .. "/autoload.txt" -- ✅ UPDATED
		...
		-- REFRESH LIST (ONLY CURRENT GAME)
		function Luna:RefreshConfigList()
			if isStudio then return "Config system unavailable." end

			local folder = Luna.Folder .. "/settings/" .. gameId -- ✅ UPDATED
